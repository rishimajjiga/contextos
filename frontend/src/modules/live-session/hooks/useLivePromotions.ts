...
export function useLivePromotions(enabled: boolean) {
  ...
  const createPromotion = useCallback(async (imageUrl: string, linkUrl: string | null, adminEmail: string) => {
    const client = getLiveClient();
    if (!client) throw new Error("Live session backend not configured.");
    const { error } = await client.from(TABLES.promotions).insert({
      image_url: imageUrl, link_url: linkUrl, is_active: true, created_by: adminEmail,
    });
